...
// HashMap represents the hash map using double hashing
type HashMap struct {
	buckets   []Entry
	capacity  int
	size      int
...
// hashFunction1 returns the first hash value for a given key
func (hm *HashMap) hashFunction1(key string) int {
	hash := 0
	for i := 0; i < len(key); i++ {
...
// hashFunction2 returns the second hash value for a given key
func (hm *HashMap) hashFunction2(key string) int {
	hash := 0
	for i := 0; i < len(key); i++ {
		hash = (hash<<7 - hash) + int(key[i])
	}
	return 1 + (hash & (hm.capacity - 1))
}
...
// insertEntry inserts an entry into the hash map's buckets using double hashing
func (hm *HashMap) insertEntry(buckets []Entry, entry Entry) {
	index := hm.hashFunction1(entry.key)
	step := hm.hashFunction2(entry.key)

	for buckets[index].key != "" {
		index = (index + step) & (hm.capacity - 1) // Double hashing to find the next available slot
	}
	buckets[index] = entry
}
...
func (hm *HashMap) Get(key string) (int, bool) {
	index := hm.hashFunction1(key)
	step := hm.hashFunction2(key)

	for hm.buckets[index].key != "" {
		if hm.buckets[index].key == key {
			return hm.buckets[index].value, true
		}
		index = (index + step) & (hm.capacity - 1) // Double hashing to find the next slot
	}
	return 0, false
}
