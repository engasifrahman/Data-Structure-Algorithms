...
// HashMap represents the hash map using linear probing
type HashMap struct {
	buckets   []Entry
	capacity  int
	size      int
...
// insertEntry inserts an entry into the hash map's buckets using linear probing
func (hm *HashMap) insertEntry(buckets []Entry, entry Entry) {
	index := hm.hashFunction(entry.key)
	for buckets[index].key != "" {
		index = (index + 1) & (hm.capacity - 1) // Linear probing to find the next available slot
	}
	buckets[index] = entry
}
...
func (hm *HashMap) Get(key string) (int, bool) {
	...
	for hm.buckets[index].key != "" {
		...
		index = (index + 1) & (hm.capacity - 1) // Linear probing to find the next slot
	}
	return 0, false
}
